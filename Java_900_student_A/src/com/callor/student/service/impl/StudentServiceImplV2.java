...
public class StudentServiceImplV2 extends StudentServiceImplV1{
	...
	public StudentServiceImplV2(String stdFile) { //파일이름을 받아서
		super(stdFile); // V1의 생성자를 호출한다
	...
	public StudentDto searchStudent(String name) {
		...
		for(StudentDto dto : stdlist) {
			 ...
			 */
			if(dto.name.contains(name.trim())) {  //이렇게 작성했을경우 이름을 입력하면 ex)수 입력한 이름이 포함된 이름을가진 모든 학생의정보가 출력됨.
				searchList.add(dto);
