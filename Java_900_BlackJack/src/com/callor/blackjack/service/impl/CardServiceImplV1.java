...
public class CardServiceImplV1 implements CardService {
	...
	//	CardDto cardDto = new CardDto();
		...
		for(int i = 0; i <100 ; i++) {
			...
			cardList.set(index1, cardList.get(index2)); //두개 요소의 위치를 바꾼다. 100번
	...
	public CardDto getCard() {
		...
		if(cardList.isEmpty()) this.makeCardDeck(); //카드가 비었으면 카드를 재생성하라
		...
		// CardDto getCard = cardList.remove(0); //한줄로 작성하는법
		// return getCard;
		
		// 별도의 변수(return 할 객체가 담긴 변수)를 만들지 않기
		return cardList.remove(0);  //바로리턴도가능
	}
