...
import com.callor.student.utils.Line2;
...
public class StudentServiceV3A {
	 ...
	 * 학번을 매개변수로 전달받아
	 * students 리스트에서 검색하여
	 * 일치하는 학생정보가 있으면 그 정보를 통째로(dto) return
	 */
	private StudentDto selectStdNum(String num) {
		...
		for(StudentDto dto : students ) {
			if(dto.num.equals(num)) return dto; //중복된 값이 있으면 
	...
	public boolean inputStudent() {
		...
		while(true) {
			...
			// selectDtdNum() method 에 학번을 보내고
			// return 값을 받아서
			// 결과 null 이 아니면 중복임을 판단하기
		//	if(this.selectStdNum(stdDto.num) != null  ) { //selectstdnum에 학번을 전달해서 null 이면 그 학번이 없단말
