...
public class ScoreServiceV1 {
	 ...
	 * scores 객체를 V2에서 사용하기 위하여
	 * protected 로 변경해준다 
	 */
	protected List<ScoreDto> scores = null; // class 어디에서나 접근 할 수있게 class 에 선언

	public ScoreServiceV1() {
		// 생성자에서 아래와 같이 scores 를 생성하면
		// 생성자를 벗어난 후 scores 는 소멸된다
		// 클래스 영역에 선언된 scores 는 여전히 null 이다
		// List<ScoreDto> scores = new ArrayList<ScoreDto>(); //이렇게 작성할 경우 이름은 scores 로
		// 같지만 완전히 다르다

		// 클래스 영역에 선언된 scores 를 "생성"하는 코드
	...
	public void makeScore() {
		...
		if (scores.size() > 0) {
			...
			strStdNum = dto.stdNum; // 임시 dto 로부터 num 값을 가져오고
			// } if 문이 여기서 끝나면 strStdNum이 23000
